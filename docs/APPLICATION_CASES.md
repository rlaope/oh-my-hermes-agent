...
| Discord dev-team routing | `이거 위험한 리팩터링 같아` | `ai-slop-cleaner` / `present_plan` | `safe-feature-change` | A selected executor/runtime handoff can be prepared for behavior-preserving cleanup after the safe plan. |
