...
from .chat_router import CHAT_SOURCES, CONFIDENCE_LEVELS, extract_message_text
from .coding_delegation import extract_source_metadata
from .coding_lifecycle import report_codex_delegation_lifecycle, start_codex_delegation_lifecycle
...
from .wrapper_contract import build_chat_interaction_payload, build_chat_status_interaction
...
def record_plan_decision(paths: OmhPaths, session_id: str, decision: str) -> dict[str, object]:
    ...
    updates = {
        "accept": {"status": "plan_accepted", "decision": "plan_accepted"},
    ...
    *,
    ...
    if session.get("current_run_id"):
        ...
        return {
            ...
            "session": session,
            "status": build_wrapper_session_status(paths, session_id),
            "handoff": report_codex_delegation_lifecycle(paths, str(session["current_run_id"])),
        }
    if session["status"] != "plan_accepted":
...
def _session_chat_response(session: dict[str, Any]) -> dict[str, object]:
    ...
    if status == "plan_accepted":
        return _chat_response(
            ...
            body="I can prepare a Codex handoff, but no executor work is observed yet.",
            phase="plan_accepted",
