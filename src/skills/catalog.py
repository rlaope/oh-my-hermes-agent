...
_DEFINITIONS = [
    ...
    SkillDefinition(
        ...
        handoff_policy="Keep loop orchestration, interviews, research, planning, feedback evaluation, status, and permission-envelope narration in Hermes; prepare selected executor handoffs only when the loop produces concrete coding work and record completion only from linked goal/runtime evidence.",
        required_inputs=("north-star goal summary", "goal reframe", "success criteria", "permission profile", "feedback or wait signal"),
        expected_outputs=("loop_cycle/v1 state", "loop_status_card/v1 next action", "executor-neutral handoff only when permitted", "external-wait or checkpoint boundary"),
        artifact_expectations=("metadata-only .omh/loops loop_cycle/v1 artifact", "loop_status_card/v1 wrapper payload", "linked goal_ledger/v1 only when completion evidence is required"),
        safety_rules=(
...
_HARNESSES = [
    ...
    HarnessDefinition(
        ...
        ("loop_cycle/v1 artifact", "loop_status_card/v1 next action", "permission envelope", "linked goal or runtime evidence references when available"),
        ("next loop step is clear", "permission boundaries are explicit", "external waiting and context exhaustion are recorded", "goal completion claims are delegated to goal_ledger/v1"),
        ("validate loop_cycle/v1", "inspect loop_status_card/v1", "check linked goal_completion_gate/v1 before completion copy"),
