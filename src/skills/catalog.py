...
_DEFINITIONS = [
    ...
    SkillDefinition(
        ...
        expected_outputs=("loop_start_card/v1 setup prompt", "loop_cycle/v1 state", "loop_status_card/v1 next action", "loop_runtime/v1 queued tick", "loop_queue_handoff/v1 only when permitted", "executor-neutral handoff only when permitted", "external-wait or checkpoint boundary"),
        artifact_expectations=("metadata-only .omh/loops loop_cycle/v1 artifact", "loop_runtime/v1 queue entries", "loop_status_card/v1 wrapper payload", "loop_start_card/v1 wrapper setup card", "linked goal_ledger/v1 only when completion evidence is required"),
        safety_rules=(
...
_HARNESSES = [
    ...
    HarnessDefinition(
        ...
        "Use when a direct loop invocation or explicit long-horizon goal needs repeated cycles until evidence, authority, context, or external waiting stops the next step.",
        ("north-star goal summary", "reframed implementable target", "success criteria", "permission profile", "feedback or wait signal"),
        ("loop_start_card/v1 setup card", "loop_cycle/v1 artifact", "loop_runtime/v1 queue entry", "loop_queue_handoff/v1 actionable handoff", "loop_status_card/v1 next action", "permission envelope", "linked goal or runtime evidence references when available"),
        ("next loop step is clear", "runtime tick queue is prepared, observed, or blocked with a reason", "permission boundaries are explicit", "external waiting and context exhaustion are recorded", "goal completion claims are delegated to goal_ledger/v1"),
        ("validate loop_cycle/v1", "inspect loop_runtime/v1 queue", "inspect loop_status_card/v1", "inspect loop_queue_handoff/v1 when a queued item is actionable", "check linked goal_completion_gate/v1 before completion copy"),
