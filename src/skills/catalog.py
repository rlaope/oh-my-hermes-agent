...
_DEFINITIONS = [
    ...
    SkillDefinition(
        ...
        expected_outputs=("loop_cycle/v1 state", "loop_status_card/v1 next action", "loop_runtime/v1 queued tick", "executor-neutral handoff only when permitted", "external-wait or checkpoint boundary"),
        artifact_expectations=("metadata-only .omh/loops loop_cycle/v1 artifact", "loop_runtime/v1 queue entries", "loop_status_card/v1 wrapper payload", "linked goal_ledger/v1 only when completion evidence is required"),
        safety_rules=(
...
_HARNESSES = [
    ...
    HarnessDefinition(
        ...
        ("loop_cycle/v1 artifact", "loop_runtime/v1 queue entry", "loop_status_card/v1 next action", "permission envelope", "linked goal or runtime evidence references when available"),
        ("next loop step is clear", "runtime tick queue is prepared or blocked with a reason", "permission boundaries are explicit", "external waiting and context exhaustion are recorded", "goal completion claims are delegated to goal_ledger/v1"),
        ("validate loop_cycle/v1", "inspect loop_runtime/v1 queue", "inspect loop_status_card/v1", "check linked goal_completion_gate/v1 before completion copy"),
