...
- `goal-loop`: Run ambitious goal loops through interview, research, planning, handoff, feedback, waiting, and resumable status without hidden execution. Tier `loop-gated`. Ladder: `loop_triggered` -> `goal_reframed` -> `permission_profile_recorded` -> `research_plan_handoff_cycle_recorded`. Actions: `choose_permission_profile`, `start_loop`, `show_loop_status`, `prepare_handoff`, `choose_executor`. Privacy `metadata_only`.
