...
class RouterContentTests(unittest.TestCase):
    ...
    def test_first_release_trust_surfaces_are_present(self) -> None:
        ...
        self.assertIn("The installer prints the installed `omh` command path", quick_start)
        self.assertIn("Python user", quick_start)
        self.assertIn("scripts directory is not on `PATH`", quick_start)
        ...
        self.assertIn("OMH_WITH_PLUGIN=1", installation)
        self.assertIn("OMH_PROFILE_PACKS=cto-loop,startup-delivery", installation)
        self.assertIn("OMH_SETUP_PROFILES=1,3", installation)
        ...
        self.assertIn("add the printed directory to `PATH`", installation)
        ...
        self.assertIn("OMH_WITH_PLUGIN=1 OMH_PROFILE_PACKS=cto-loop", install_for_agents)
        ...
        self.assertIn('OMH_PIP_ARGS="${OMH_PIP_ARGS---user}"', install_sh)
        self.assertIn("find_omh_command", install_sh)
        self.assertIn("Installed omh command:", install_sh)
        ...
        self.assertIn("oh_my_hermes_agent-1.0.0-py3-none-any.whl", release)
        self.assertIn("/tmp/omh-wheel-smoke/bin/omh --help", release)
        self.assertIn("OMH_PIP_ARGS=", release)
